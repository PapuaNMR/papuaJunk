...
while line_num < len(shifts_ppm):
	...
	try:	
		params = curve_fit(pyruvate_func, x, Ca, 
				p0=([0.35*max, max, 1.5, 16.5, 4.3]),
			
			#	sigma=1.0/np.log(Ca),
			#	absolute_sigma=False,
			
				bounds=([0.000*max, 0.5*max, 1.0, 14, 3.0],[max, 2*max, 2.0, 17, 5.8]),
				method='dogbox',
				max_nfev=10000
				)
		...
		if SDerr[0]/p[0] <= 0.5:
			...
			plt.figtext(.15,.70,'s ='+'%.3E' % p[2]+' +/- '+'%.3E' % SDerr[2], **fit_font)
			plt.figtext(.15,.65,'cen='+'%.3E' % p[3]+' +/- '+'%.3E' % SDerr[3], **fit_font)
			plt.figtext(.15,.60,'dis='+'%.3E' % p[4]+' +/- '+'%.3E' % SDerr[4], **fit_font)
			...
			plt.savefig('fittings_s/'+aa+num+'.png')
			plt.close()
		
			res = sequence[int(shifts_ppm[line_num][0])-1]
			spectra_amino_dic[res].append(p[4])
...
plt.savefig('fittings_s/distros.pdf')
plt.show()
...
#print np.outer(A, B)a
	...
	plt.savefig('fittings_s/Space_For_'+three_aa+'.pdf')
	plt.close()
